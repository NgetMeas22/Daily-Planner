...
$stmt = $conn->prepare('SELECT deep_work, daily_reminders, focus_duration FROM settings WHERE user_id = ?');
$stmt->bind_param('i', $userId);
$stmt->execute();
...
if (!$settings) {
    ...
    $stmt->bind_param('i', $userId);
    $stmt->execute();
    $settings = ['deep_work' => 0, 'daily_reminders' => 1, 'focus_duration' => 45];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_settings'])) {
    if (!csrf_valid()) {
        ...
        $stmt = $conn->prepare('UPDATE settings SET deep_work = ?, daily_reminders = ?, focus_duration = ?, updated_at = NOW() WHERE user_id = ?');
        $stmt->bind_param('iiii', $deepWork, $reminders, $focus, $userId);
        $stmt->execute();
...
<div class="container py-4 py-md-5" style="max-width: 780px;">
    <div class="d-flex align-items-center gap-3 mb-4">
        <h2 class="h3 fw-bold mb-0">Settings</h2>
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-4">
            <h6 class="section-title text-uppercase text-muted mb-3 small">Account</h6>
            <div class="d-flex align-items-center gap-4 flex-wrap">
                ...
                <div class="flex-grow-1">
                    ...
                    <a class="btn btn-sm btn-outline-primary" href="profile.php"><i class="bi bi-person-gear me-1"></i>Edit profile (name, photo, password)</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-4">
            <h6 class="section-title text-uppercase text-muted mb-3 small">Preferences</h6>
            <form method="post">
                ...
                <div class="mb-3" style="max-width: 320px;">
                    <label class="form-label text-secondary small fw-medium" for="focusDuration">Default Focus Duration</label>
                ...
                <button type="submit" class="btn btn-primary fw-bold px-4 rounded-3">Save Preferences</button>
            </form>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-4">
            <h6 class="section-title text-uppercase text-danger mb-3 small">Danger Zone</h6>
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <div>
                    <h6 class="mb-1 fw-semibold text-dark">Delete Account</h6>
                ...
                <form method="post" class="w-100" onsubmit="return confirm('Are you absolutely sure? This permanently deletes your account and ALL data. This cannot be undone!');">
                    ...
                    <div class="d-flex flex-column flex-sm-row align-items-sm-center gap-2">
                        <input type="password" name="delete_password" class="form-control rounded-3" placeholder="Enter your password" required style="max-width: 260px;">
