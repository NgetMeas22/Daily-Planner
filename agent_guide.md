...
**budget_months** — per-month budget carry-over bookkeeping: `id`, `user_id` (FK→users, CASCADE), `budget_month` (CHAR(7) `'YYYY-MM'`), `base_budget` (DECIMAL — snapshot of `users.monthly_budget` for that month), `carry_in` (DECIMAL — leftover carried in from the previous month), `created_at`. `UNIQUE (user_id, budget_month)`. Gives the expenses page its monthly rollover: leftover money from the previous month (`carry_in + base_budget + income − expenses`, floored at 0) is carried into the next month's budget automatically.
...
- **Remaining budget:** `max(0, effectiveBudget - monthly_expenses + monthly_income)` for the month of the selected date (`expenses.php`). `effectiveBudget = base_budget + carry_in`, where `carry_in` is the leftover rolled forward from the previous month (`budget_months`). `expense_budget_get($month, $monthlyBudget)` in `expenses.php` returns `[base_budget, carry_in]` and lazily creates a row for a month (carrying in the prior month's surplus, or 0 when the prior month has no tracked row). Editing the monthly budget updates the displayed month's `base_budget` immediately (keeps `carry_in`).
